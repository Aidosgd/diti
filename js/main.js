$(document).ready(function () {
    ...
    $('#el-1').click(function() {
        if($('#el-1').is(':checked')) { $('.container').addClass("shadoz"); }
        if(!$('#el-1').is(':checked')) { $('.container').removeClass("shadoz"); }
    });
    $('#el-3').click(function() {
    ...
    $(document).on('click', '.uploadImage', function () {
        ...
        $(".container").css('background','url('+ sorc +')');
        $(".container").css('background-size','contain');
        $(".container").css('background-repeat','no-repeat');
        var parts = sorc.split('/');
        name = parts[parts.length-1];
        namez = name.split('.');
        zaq = urldecode(namez[0]);
        var mess = "Вы выбрали двери: <a href='"+ linz +"'>"+ zaq +"</a>";
        $(".fabinfo").html(mess);
        return false;
    });
